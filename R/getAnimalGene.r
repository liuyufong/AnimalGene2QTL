...
#'
#' @param gene_attributes, Attributes you want to retrieve.
#' A possible list of gene_attributes can be retrieved using the function
...
if(nrow(singleRowQuery) < 1){
geneNA[1:attlength] <- "NO this 'value' in QTL database";
geneNA <- as(geneNA, "list");
if(is.list(qtl_values)){
qtlgene <- data.frame(geneNA, qtl_values[i,]);
colnames(qtlgene)[1:(attlength+flength)] <- c(gene_attributes,
