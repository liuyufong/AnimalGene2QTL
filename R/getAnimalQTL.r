...
#' @param gene_filters, Filters (one or more) that should be used in
#' the query. A possible list of filters can be retrieved using the
#' function listGeneAF().
#' @param gene_values, Values of the filter, e.g. vector of IDs. If
...
NULL_QTL[(NCOL(NULL_QTL)+1):
(NCOL(NULL_QTL)+qtlattlength+1)] <- NA;
NULL_QTL[(qtlattlength+1):
(qtlattlength+gflength)] <- NULL_QTL[1:gflength];
NULL_QTL[1:qtlattlength] <- NA;
colnames(NULL_QTL) <- c(qtl_attributes,
gene_filters,
"length in the region (bp)");
result <- NULL_QTL;
if(snp == TRUE){
result[(NCOL(result)+1):
(NCOL(result)+snpattlength+1)] <- NA;
colnames(result) <- c(qtl_attributes,
gene_filters,
"length in the region (bp)",
...
for(i in 1:chromrow){
query1 <- paste(a1, att, a2, qtldatabase,
...
if(Nrow >= 1){
single_QTL1[(NCOL(single_QTL1)+1):
(NCOL(single_QTL1)+gflength)] <- chromchr[i,1:gflength];
single_QTL1[NCOL(single_QTL1)+1] <- "ALL";
colnames(single_QTL1) <- c('Chstart',
'Chend',
qtl_attributes,
gene_filters,
"length in the region (bp)");
if(snp == TRUE){
if(snprow>=1){
sg_SNP <- single_SNP[which(single_SNP[snpattlength+1]
==chromchr[i,gflength+1]),];
for(k in 1:Nrow){
sg_SNP1 <- sg_SNP[which(sg_SNP[snpattlength+2]
>=single_QTL1[k,1]),];
sg_SNP1 <- sg_SNP1[which(sg_SNP1[snpattlength+3]
<=single_QTL1[k,2]),];
sg_SNP2 <- sg_SNP[which(sg_SNP[snpattlength+2]
<single_QTL1[k,1]),];
sg_SNP3 <- sg_SNP[which(sg_SNP[snpattlength+3]
>single_QTL1[k,2]),];
sg_SNP1 <- sg_SNP1[1:snpattlength];
sg_SNP2 <- sg_SNP2[1:snpattlength];
sg_SNP3 <- sg_SNP3[1:snpattlength];
sg_SNP1[(NCOL(sg_SNP1)+1):(NCOL(sg_SNP1)+2)] <- single_QTL1[k,1:2];
sg_SNP2[(NCOL(sg_SNP2)+1):(NCOL(sg_SNP2)+2)] <- single_QTL1[k,1:2];
sg_SNP3[(NCOL(sg_SNP3)+1):(NCOL(sg_SNP3)+2)] <- single_QTL1[k,1:2];
...
gc();
if(NROW(snqresult)>0){
snqresult[(NCOL(snqresult)+1):(NCOL(snqresult)+
qtlattlength+gflength+1)] <- 'NA';
snqresult[(qtlattlength+gflength+2):
NCOL(snqresult)] <- snqresult[1:(snpattlength+2)];
snqresult[1:(qtlattlength+gflength+1)] <- NA;
colnames(snqresult) <- c(qtl_attributes,gene_filters,
"length in the region (bp)",snp_attributes,'Chstart','Chend');
snqresult <- as.data.frame(lapply(snqresult[1:NCOL(snqresult)],unlist));
...
sg_SNP2 <- sg_SNP2[1:snpattlength];
sg_SNP3 <- sg_SNP3[1:snpattlength];
sg_SNP1[(NCOL(sg_SNP1)+1):(NCOL(sg_SNP1)+2)] <- single_QTL2[k,1:2];
sg_SNP2[(NCOL(sg_SNP2)+1):(NCOL(sg_SNP2)+2)] <- single_QTL2[k,1:2];
sg_SNP3[(NCOL(sg_SNP3)+1):(NCOL(sg_SNP3)+2)] <- single_QTL2[k,1:2];
colnames(sg_SNP1) <- c(snp_attributes,'Chstart','Chend');
colnames(sg_SNP2) <- c(snp_attributes,'Chstart','Chend');
colnames(sg_SNP3) <- c(snp_attributes,'Chstart','Chend');
snpresult <- rbind(snpresult,sg_SNP1);
...
single_QTL2[(NCOL(single_QTL2)+1):
(NCOL(single_QTL2)+gflength)] <- chromchr[i,1:gflength];
...
gc();
if(NROW(snqresult)>0){
snqresult[(NCOL(snqresult)+1):
(NCOL(snqresult)+qtlattlength+gflength+1)] <- 'NA';
snqresult[(qtlattlength+gflength+2):
NCOL(snqresult)] <- snqresult[1:(snpattlength+2)];
snqresult[1:(qtlattlength+gflength+1)] <- NA;
colnames(snqresult) <- c(qtl_attributes,
gene_filters,"length in the region (bp)",
...
sg_SNP <- single_SNP[which(single_SNP[snpattlength+1]==
chromchr[i,gflength+1]),];
sg_SNP <- sg_SNP[which(sg_SNP[snpattlength+2]>=chromchr[i,gflength+2]),];
sg_SNP <- sg_SNP[which(sg_SNP[snpattlength+3]<=chromchr[i,gflength+3]),];
