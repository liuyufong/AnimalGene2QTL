...
#'
#' @param snp_attributes, Attributes you want to retrieve.
#' A possible list of snp_attributes can be retrieved using the function
#' listAttributes {biomaRt}.
#' @param qtl_filters, qtl_filters (one or more) that should be used in
#' the query. A possible
#' list of filters can be retrieved using the function listFilters.
#' @param qtl_values, Values of the qtl_filters, e.g. vector of IDs.
#' If multiple qtl_filters are specified then the argument should be a list
#' of vectors of which the position of each
#' vector corresponds to the position of the qtl_filters in the qtl_filters
#' argument.
...
getSNPbyQTL <- function(snp_attributes,
qtl_filters, qtl_values, data_set){
if(missing(snp_attributes))
stop("Argument 'snp_attributes' must be specified.")
if(is.list(qtl_filters) && !missing(qtl_values))
warning("Argument 'qtl_values' should not be used when argument
'qtl_filters' is a list and will be ignored.")
...
values=list(qtlquery[,qtlflength+1],qtlquery[,qtlflength+2],
qtlquery[,qtlflength+3]),
mart=mart);
snprow <- NROW(snp_all);
...
sg_snp[(snpattlength+1):(snpattlength+2)] <- qtlquery[k,
(qtlflength+2):(qtlflength+3)];
...
result <- "There is NULL QTL database by searching for input QTL data.please
check your data.";
}
dbDisconnect(con);
